...
3. **Fold it.** Invoke `survey-explainer-fold` (modes: `full` / `note-only` / `prose`; ask only if genuinely ambiguous — the reader's last few folds are the best default). The fold is the session's durable output.
4. **Watch for the escalations** and name them when they appear:
   - the question needs a *number* the corpus does not have → an **experiment**; scope it to the ~5 USD envelope and run it, or file a `todos/` entry.
