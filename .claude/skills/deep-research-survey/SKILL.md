...
- **`max`** (meta-mode preset) — the maximum depth + length setting. Expands to **`proposed` + `scale: wide` + `audience: learner`** and is treated as exactly that expansion by ALL downstream logic (addenda lazy-loading below, the `config/operational-scale.json` read at Phase-3 launch, the `config/audience-register.json` resolution at Phase 1): every P-item and richness/depth item ON, the `wide` throughput profile, and the pedagogical `learner` register (fundamentals pinned to the `headline` tier). An explicit axis argument passed alongside `max` overrides just that axis (e.g. `max, audience: expert` keeps `proposed` + `wide` but uses the expert register). `max` inherits the `scale: wide` contract unchanged — it is INVALID unless every `safety_net_invariant` in `config/operational-scale.json` stays ON (all default-ON), and it carries wide mode's provisional caveat (searches=40 is above the only measured death boundary near 28; recoverable via the retry + coverage-gap safety nets, not eliminated). Record the resolved expansion (`max -> proposed / wide / learner`) in the Phase-5 report footer next to mode/flags/scale/audience.

**Lazy loading (token discipline).** The improvement addenda are NOT inlined — they live in `addenda/` and cost zero tokens unless a non-`original` mode is active. When `proposed`, `richness`, or `flags:` is set: read `addenda/global.md` once (P2-4, P2-5), then read `addenda/phase-N.md` just-in-time as you begin phase N — and only if `proposed`/`richness` is set or one of that phase's item ids (named in each phase file's one-line pointer) is in the active `flags` set. Apply a block iff `proposed`/`richness` is set OR its id is in `flags`. The Part-2 richness items live in `addenda/phase-2.md` (R-GOV / R-CARD / R-SURVEY), `addenda/phase-4.md` (R-DEPTH), and `addenda/phase-5.md` (R-MATHREV / R-COVER / R-RUBRIC); `richness` activates all of them and `R-GOV` (the depth-tier governor) must be read first. Record the active mode/flags in the final report (one line). Item registry + how each is tested: `bench/deep-research-survey/items.json`. Default stays `original` — no `addenda/` file is ever read. **Separately**, individually-toggleable post-hoc refinements that are NOT part of this mode lattice (workflow/gotcha/rule corrections) are registered in `.claude/skill-options.json` and marked `[opt:<ID> …]` at their point of use across this skill, `citation-audit`, and the cross-linking / math-authoring rules; each has its own `default` (on/off) you flip there.
...
- **Search-budget starvation is NOT the step cap, and the symptoms look identical.** A cluster that returns thin may have run out of *steps* (per-agent, ~36–40 tool calls — the gotcha above) or out of *searches* (`searches_per_session`, a harness-enforced ~200 **shared across the main thread and every subagent**). The per-agent knob does not bound the fan-out: 10 agents at wide=40 request 400 against a 200-call pool. Measured upstream — the pool emptied mid-run, three clusters reported it explicitly, and the main thread had none left for Phase-4/5 verification. Size with `min(searches_per_agent, (searches_per_session - holdback) / n_agents)` and keep the holdback. Diagnose by which resource the agent's own output says it hit, not by which cause is more familiar (`CLAUDE.md` § Agent Fan-Out — Sizing and Failure Diagnosis).
...
- **Phase-4 authoring: write each prose paragraph as a single unwrapped line** (or at least never break a line immediately before a `<!-- ref:/secref:/secxref:/cite: -->` comment or inside a `$...$` span). The PostToolUse `lint-math` hook rejects a ref-comment or split inline-math at line start — the #1 synthesis round-trip sink, since hard-wrapping at ~100 cols repeatedly lands a marker or math at a continuation line's start. A deterministic joiner (read each line; if it starts with a ref-comment, append it to the previous line) clears a whole file in bulk. **[opt:SX-INIT · default ON · toggle `.claude/skill-options.json`]** **Author same-survey `§` refs BARE (`§X.Y`), never pre-linked as `[§X.Y](file#sec-X.Y)`.** Bare `§X.Y` is ERROR-blocking when `.claude/bare-refs-severity` is `error`, so the PostToolUse hook errors on each write — but the file still lands on disk, so DON'T fix per-file. Author every file, then run **`/normalize-survey <dir>` once** (`viewer/tools/normalize-survey.py` — the write-mode twin of `/check-survey`): it runs `renumber-sections --init` (which converts every bare same-survey `§X.Y` to `secref` same-file / `secxref` cross-file via `order.json`), the rest of the renumber/link sequence in the correct order (references.md excluded from paragraph-init), and the full check suite — reporting any unresolved bare `§` as `SX-DEGLYPH` candidates. Pre-linking `[§X.Y](...)` *without* the marker is the trap that forces a per-file round-trip, because `--init` fixes **bare** refs, not linked-but-unmarked ones; a custom marker-inserter is unnecessary — `--init` already does the job. TWO ref kinds must **never** be written bare, because `--init` would mis-resolve them to a same-numbered section of *this* survey: a **cross-SURVEY** `§` (a link into a *different* survey, not in this `order.json`) → de-glyph to a plain relative link with **no** `§` glyph (`[interpretability survey, circuits section](../mechanistic-interpretability/circuits-across-models.md#sec-9.2)`); an **external-spec** `§` → bracket-wrap (`[MCP spec §6.2]`, `[§14.6]`).
