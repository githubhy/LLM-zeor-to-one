...
- **Estimated searches per agent**: `min(searches_per_agent, (searches_per_session - holdback) / n_agents)` — **the per-agent knob is not the binding limit at wide scale.** `searches_per_agent` (standard 15 / wide 40) bounds ONE agent; `searches_per_session` (measured **200**) is a harness-enforced pool **shared across the main thread and every subagent**, and no flag raises it. N agents sized only by the per-agent knob REQUEST 40N and silently degrade once the pool empties. **Measured 2026-07-27:** 10 agents at wide=40 requested 400 against a 200-call pool, exhausted it mid-run, and left the main thread **zero** searches for Phase-4/5 verification; correct sizing was `min(40, (200-25)/10) = 17`. **Reserve the holdback (20–30 calls) for the main thread** — synthesis that cannot check itself is the expensive failure here, not a thin cluster.
