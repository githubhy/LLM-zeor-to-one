...
- `/study [<topic|question>]` — Run a **study session**: open with **retrieval practice** (`--recall`: one old fold re-answered from memory — fluency with the corpus open is not storage strength), then a computed frontier menu (`viewer/tools/study-frontier.py`, ranked by fold density — least-interrogated first, and **gated to one rung above the reader** via `.claude/study-prereqs` + `.claude/study-reader-rung`; held-back documents are reported, never silently dropped). The reader picks or substitutes, then read → ask → fold, escalating to an experiment / `/enrich` / a new survey when a question demands it. Closes with the pulse check (`--since`: **zero folds means the session did not happen**) and a log entry whose payload is the *questions asked*. Autonomous end-to-end mode is out of bounds inside a topic-month. Defined in `.claude/commands/study.md`; contract in `CONTEXT.md` + `decisions/2026-08-13-02`.
- `/wait-what` — Reader-invoked escape hatch inside a `/study` session: re-pitch the last answer in ASD-STE100 Simplified Technical English using `CONTEXT.md`'s vocabulary, leading with the signal-processing reading. A re-pitch, never a longer answer. Defined in `.claude/commands/wait-what.md`.
