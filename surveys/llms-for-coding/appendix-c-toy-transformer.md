## <a id="sec-C"></a>A Transformer Top-to-Neuron: a Fully Worked Toy Model
...
> <a id="p-c5-validation-and-the-toy-actually-learns-4"></a><!-- para:c5-validation-and-the-toy-actually-learns-4 --> **Note — what "learned" does and doesn't mean here.** The 3-cycle is the successor map $y_t=(x_t+1)\bmod 3$ on the cyclic group $\mathbb{Z}/3\mathbb{Z}$ — the one-argument ancestor of the modular addition that groks in <!-- secref:C.8 -->[§C.8](#sec-C.8). And because training uses a single length-3 sequence, "learned" here means *memorized*: there is no held-out data, and position and token content stand in bijection, so the gradient check and near-zero loss certify the mechanism is correct but cannot say *which* circuit the model used or whether it would transfer. These boundaries, and other things the toy quietly fixes, are collected in <!-- secref:C.10 -->[§C.10](#sec-C.10).

> <a id="p-c5-validation-and-the-toy-actually-learns-5"></a><!-- para:c5-validation-and-the-toy-actually-learns-5 --> **Note — the same machinery can *grok*.** Here the toy learns its 3-cycle quickly and monotonically. Run the *same* architecture and optimizer on modular addition, $c=(a+b)\bmod p$, with weight decay, and the curve changes character: training loss reaches zero early while validation accuracy stays at chance, then — orders of magnitude more steps later — validation *suddenly* jumps to near-perfect. This delayed generalization is **grokking**; why it happens, derived from first principles, is <!-- secref:C.8 -->[§C.8](#sec-C.8).
...
<a id="p-c8-grokking-why-generalization-can-lag-memorization-17"></a><!-- para:c8-grokking-why-generalization-can-lag-memorization-17 --> **Provenance.** The empirical results this section builds on — the phenomenon of grokking, the Fourier-multiplication circuit, the progress-measure methodology, and the circuit-efficiency account of the delay — are attributed at each claim above. The derivation itself is deliberately self-contained: those references credit the empirical findings, and no step of the derivation rests on them.
...
> <a id="p-c9-weight-decay-and-the-adamw-decoupling-4"></a><!-- para:c9-weight-decay-and-the-adamw-decoupling-4 --> **Note — why the gradient is $\lambda\theta$, and why it points inward.** The penalty $\tfrac{\lambda}{2}\lVert\theta\rVert^2$ differentiates to $\lambda\theta$ — the $\tfrac{1}{2}$ is there precisely to cancel the $2$ brought down by the square. Descent steps *against* the gradient, so this term contributes $-\eta\lambda\theta$: a vector antiparallel to $\theta$, pointing from the current weights straight back to the origin, with magnitude $\lambda\lvert\theta_i\rvert$ on each coordinate — large weights pulled hard, near-zero weights barely touched. Picture a spring tethering every weight to $\mathbf{0}$; applied on its own it is exactly the shrink $\theta\leftarrow(1-\eta\lambda)\,\theta$ of Equation <!-- ref:C-19 -->[(19)](#eq-19).

<a id="p-c9-weight-decay-and-the-adamw-decoupling-5"></a><!-- para:c9-weight-decay-and-the-adamw-decoupling-5 --> **Why Adam needs the "W".** They stop being the same once the optimizer rescales the gradient per-coordinate. Adam divides by $\sqrt{\hat{\mathbf{v}}}+\epsilon$ in Equation <!-- ref:C-10 -->[(10)](#eq-10); feeding the $\lambda\theta$ term through that same division makes the effective decay of each weight depend on its recent gradient magnitude — large-gradient weights get *less* decay, the opposite of what a clean norm penalty intends. **AdamW** fixes this by *decoupling* the decay: take the adaptive step, then shrink the weights directly,
...
<a id="p-c9-weight-decay-and-the-adamw-decoupling-6"></a><!-- para:c9-weight-decay-and-the-adamw-decoupling-6 --> which restores the uniform $(1-\eta\lambda)$ shrink of Equation <!-- ref:C-19 -->[(19)](#eq-19). This is the "W" in AdamW, and it is why every configuration in this series specifies AdamW rather than Adam with an L2 term folded into the loss.

<a id="p-c9-weight-decay-and-the-adamw-decoupling-7"></a><!-- para:c9-weight-decay-and-the-adamw-decoupling-7 --> **What it buys.**

- <a id="p-c9-weight-decay-and-the-adamw-decoupling-8"></a><!-- para:c9-weight-decay-and-the-adamw-decoupling-8 --> A simplicity prior: minimizing $\tfrac{\lambda}{2}\lVert\theta\rVert^2$ is the maximum-a-posteriori estimate under a zero-mean Gaussian prior on the weights — "prefer the smallest weights that still fit."
...
<a id="p-c9-weight-decay-and-the-adamw-decoupling-9"></a><!-- para:c9-weight-decay-and-the-adamw-decoupling-9 --> **Intuition.** Gradient descent pushes weights wherever the data wants them; weight decay is a constant, gentle tide pulling them all back toward zero. The equilibrium is the smallest set of weights that still explains the training data — which is usually the solution that also explains data it has never seen.

<!-- sec:C.10 -->
### <a id="sec-C.10"></a>C.10 What Else the Toy Reveals — and What It Cannot

<a id="p-c10-what-else-the-toy-reveals-and-what-it-cannot-1"></a><!-- para:c10-what-else-the-toy-reveals-and-what-it-cannot-1 --> The toy was built to make one *mechanism* visible: a complete forward pass, a complete backward pass, and one optimizer step. Read instead as a *specimen* rather than a mechanism, the same tiny model fixes several other things sharply — some encouraging, some cautionary. Each recurs, magnified, at scale, so they are worth naming before the ladder leaves the toy behind.

<a id="p-c10-what-else-the-toy-reveals-and-what-it-cannot-2"></a><!-- para:c10-what-else-the-toy-reveals-and-what-it-cannot-2 --> **The task is the successor map on a cyclic group.** The training pair $x=(0,1,2)\to y=(1,2,0)$ of <!-- secref:C.1 -->[§C.1](#sec-C.1) is not an arbitrary permutation: it is the successor — "add one" — map $y_t=(x_t+1)\bmod 3$ on the cyclic group $\mathbb{Z}/3\mathbb{Z}$. The modular addition that groks in <!-- secref:C.8 -->[§C.8](#sec-C.8), $c=(a+b)\bmod p$, is the *two-argument* generalization of exactly this operation on exactly this kind of group: the toy advances one fixed step around a circle of three points; the grokking model advances a learned $b$ steps around a circle of $p$. So <!-- secref:C.5 -->[§C.5](#sec-C.5) and <!-- secref:C.8 -->[§C.8](#sec-C.8) are the same algebraic object at two rungs, and the Fourier-multiplication circuit of <!-- secref:C.8 -->[§C.8](#sec-C.8) is what this successor map becomes once the group is large enough that storing the table is no longer the cheapest way to be right.

<a id="p-c10-what-else-the-toy-reveals-and-what-it-cannot-3"></a><!-- para:c10-what-else-the-toy-reveals-and-what-it-cannot-3 --> **Mechanism, not generalization — and the circuit is unidentifiable.** There is a subtlety hiding in the phrase "the toy has learned to predict the 3-cycle." Training uses a *single* sequence of length $T=3$, so there is no held-out data and nothing to generalize to: the toy *memorizes* one sequence. And on that one sequence the two natural strategies are indistinguishable. Position-to-token is a bijection ($t=1,2,3$ carry $x_t=0,1,2$) and token-to-next is a bijection, so a circuit that predicts $y_t$ from the *position* $t$ and a circuit that predicts it from the *token content* $x_t$ produce identical logits and identical loss. The $1.6\times10^{-9}$ gradient check and the near-zero training loss of <!-- secref:C.5 -->[§C.5](#sec-C.5) therefore certify two things — that the forward/backward/optimizer are *correct*, and that the model *can fit* the data — and are silent on a third: *which* circuit it used, and whether that circuit would transfer. Separating position from content, and measuring genuine generalization, is precisely what a train/validation split and a richer dataset buy, and why the induction and grokking studies require them.

<a id="p-c10-what-else-the-toy-reveals-and-what-it-cannot-4"></a><!-- para:c10-what-else-the-toy-reveals-and-what-it-cannot-4 --> **One block cannot host an induction head.** The instantiated toy is a single block — one layer of attention. The induction head this anatomy series is really chasing (<!-- secxref:A.9 -->[§A.9](appendix-a-qkv-first-principles.md#sec-A.9)) is a *two-layer* circuit: a previous-token head in the first layer writes "what preceded me" into the residual stream, and a copy head in the second layer reads that to attend back to the earlier occurrence and predict what followed it — composition through the intervening layer (<!-- secxref:A.18 -->[§A.18](appendix-a-qkv-first-principles.md#sec-A.18)). A one-layer attention model can express the *skip-trigrams* of <!-- secxref:A.8 -->[§A.8](appendix-a-qkv-first-principles.md#sec-A.8) — direct source-token to next-token rules — but not induction, which has no single-layer form. So <!-- secref:C.7 -->[§C.7](#sec-C.7) is right that the toy's *architecture family* (causal, decoder-only) is the one that hosts induction, while this specific one-block instance sits one rung below it. That gap is the spine of the companion study's ladder: one layer to confirm skip-trigrams succeed, two layers to make the induction head appear.

<a id="p-c10-what-else-the-toy-reveals-and-what-it-cannot-5"></a><!-- para:c10-what-else-the-toy-reveals-and-what-it-cannot-5 --> **Over-parameterized interpolation, and a solution chosen by implicit bias.** The toy carries 203 parameters against three position-examples of a 3-way classification — it is heavily over-parameterized, so the settings that reach zero training loss form not a point but a continuous manifold, the same $\mathcal{M}_0$ on which the grokking argument of <!-- secref:C.8 -->[§C.8](#sec-C.8) plays out. Which point on that manifold the optimizer selects is fixed not by the data — every point fits equally — but by the *implicit bias* of the optimizer and the initialization. In the base configuration of <!-- secref:C.5 -->[§C.5](#sec-C.5) there is no weight decay, and, training on one full-batch sequence, no minibatch gradient noise, so that bias is the pure implicit bias of Adam from the given init. Add weight decay and you introduce the *explicit* norm-selection force of <!-- secref:C.9 -->[§C.9](#sec-C.9), whose slow drift toward the minimum-norm interpolant is the entire grokking transition. The clean monotone descent of <!-- secref:C.5 -->[§C.5](#sec-C.5) and the delayed jump of <!-- secref:C.8 -->[§C.8](#sec-C.8) are thus one over-parameterized-interpolation picture with the selection pressure switched off and on.

<a id="p-c10-what-else-the-toy-reveals-and-what-it-cannot-6"></a><!-- para:c10-what-else-the-toy-reveals-and-what-it-cannot-6 --> **Structural residues.** A few smaller facts the toy's chosen dimensions expose directly:

- <a id="p-c10-what-else-the-toy-reveals-and-what-it-cannot-7"></a><!-- para:c10-what-else-the-toy-reveals-and-what-it-cannot-7 --> *Gauge freedoms at the readout.* The final LayerNorm removes two degrees of freedom — the mean and the scale (<!-- secref:C.3 -->[§C.3](#sec-C.3)) — and the output softmax is invariant to a constant added to all logits, so of the $d=4$ readout directions only about two independent contrasts actually move the $V=3$-way distribution. The gradient's "blindness" along the mean and scale directions in <!-- secref:C.3 -->[§C.3](#sec-C.3) is the same fact viewed from the backward pass.
- *Room to avoid superposition.* Here $d=4>V=3$, so each token can be given its own direction in the residual stream — the toy sits in the roomy, monosemantic regime, part of why it learns without interference. Real models invert the ratio ($d\ll V$; GPT-2 small is $768$ against a $50{,}257$-token vocabulary) and must pack more features than they have dimensions — *superposition* — which is where polysemantic neurons come from. The toy shows the clean case the large models cannot afford; the geometry and capacity of superposition are derived in <!-- secxref:I.2 -->[§I.2](appendix-i-mechanistic-interpretability.md#sec-I.2).
- *Degenerate early positions.* Under the causal mask the first position attends to a single element, so its softmax is the identity and its attention sublayer is a no-op; the head is genuinely exercised only at positions 2 and 3. In a length-$T$ causal stream the attention circuit does more work the later the position.
- *Untied embeddings and the bias base-rate.* The toy keeps $E$ and $W_U$ as separate tensors, whereas GPT-2 ties them ($W_U=E^\top$); and the output bias $\mathbf{b}_U$ is the slot that would absorb the unigram base-rate of the targets (uniform on the 3-cycle, so it stays near zero). Both are concrete places the toy differs from the model it calibrates toward.

<a id="p-c10-what-else-the-toy-reveals-and-what-it-cannot-8"></a><!-- para:c10-what-else-the-toy-reveals-and-what-it-cannot-8 --> **What it buys.**

- <a id="p-c10-what-else-the-toy-reveals-and-what-it-cannot-9"></a><!-- para:c10-what-else-the-toy-reveals-and-what-it-cannot-9 --> A precise reading of the validation result: the gradient check and loss curve certify *mechanism and optimization*, and the honest limits of a single-sequence, single-block specimen are stated rather than glossed over.
- Two live bridges into the rest of the appendix — the successor-map task into the grokking circuit of <!-- secref:C.8 -->[§C.8](#sec-C.8), and the over-parameterized manifold into the weight-decay selection of <!-- secref:C.9 -->[§C.9](#sec-C.9).
- A depth-accounting that lines the toy up with the companion study's ladder: skip-trigrams at one layer, induction at two.

<a id="p-c10-what-else-the-toy-reveals-and-what-it-cannot-10"></a><!-- para:c10-what-else-the-toy-reveals-and-what-it-cannot-10 --> **Intuition.** The toy is a fully lit dissection room, not a wildlife documentary: every gradient and every parameter is visible precisely because the specimen is tiny, fixed, and over-provisioned. That is its whole value — nothing is hidden — and equally its boundary: on a single example memorization is indistinguishable from understanding, one block is one layer short of the circuit the series is chasing, and roominess ($d>V$) spares it the superposition that defines the real thing. The next rungs of the ladder exist to remove exactly these conveniences, one at a time.
