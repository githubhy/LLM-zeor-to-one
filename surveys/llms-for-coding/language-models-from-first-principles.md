...
> <a id="p-35-how-a-model-is-trained-4"></a><!-- para:35-how-a-model-is-trained-4 --> **Note — why this is $O(T)$, not $O(T^2)$.** A cacheless decoder reprocesses the whole length-$t$ prefix at every step, so the work is $\sum_{t=1}^{T} t = T(T{+}1)/2 = O(T^2)$. But the causal mask makes the past keys/values *invariant* to later tokens, so they can be cached and reused — each new token then costs one block pass, for $T = O(T)$ total. (The attention scan over the cached keys is $O(t)$ per token either way; the cache removes the prefix *recomputation*.) Full derivation and diagrams in <!-- secref:3.5.1 -->[§3.5.1](#sec-3.5.1).

<a id="p-35-how-a-model-is-trained-5"></a><!-- para:35-how-a-model-is-trained-5 --> Decoding choices (greedy, temperature sampling, top-p) and serving optimizations are detailed in Section 10.

<a id="p-35-how-a-model-is-trained-6"></a><!-- para:35-how-a-model-is-trained-6 --> ![Two panels. Left: a log-scale bar chart of training tokens per stage for DeepSeek-Coder — a tall pretraining bar at two trillion tokens, a bar a thousand times shorter for supervised fine-tuning at two billion, faint reference dots for other code models, and an annotation that RL/preference alignment is smaller still and rarely reported in tokens. Right: a log-log plot of cumulative transformer block-passes versus number of generated tokens, with an uncached quadratic curve far above a cached linear curve, annotated with the roughly four-thousand-fold reduction at a context length of 8192.](figures/training-and-serving-economics.svg)

<!-- sec:3.5.1-figure-3.5 -->
<a id="sec-3.5.1-figure-3.5"></a><a id="p-35-how-a-model-is-trained-7"></a><!-- para:35-how-a-model-is-trained-7 --> **Figure 3.5.** The two economics of training and serving. *Left — where training compute goes:* the stages have wildly different budgets, and since compute scales as $C \approx 6ND$ (parameters $\times$ tokens), at fixed model size the per-stage *token* counts are the per-stage *compute*. DeepSeek-Coder reports both of its stages in tokens — pretraining (the cross-entropy objective of Equation <!-- ref:3-2 -->[(2)](#eq-2)) on $2\times 10^{12}$ tokens versus instruction-tuning on $2\times 10^{9}$, a $1000\times$ gap, which is why almost all compute is spent in pretraining <!-- cite:10 --> [[10]](references.md#ref-10). The pattern holds across open code LLMs (Qwen2.5-Coder $5.5\mathrm{T}$ <!-- cite:11 --> [[11]](references.md#ref-11), StarCoder2 $3.3$–$4.3\mathrm{T}$ <!-- cite:9 --> [[9]](references.md#ref-9) pretraining; Code Llama $\approx 5\mathrm{B}$ instruction tokens <!-- cite:6 --> [[6]](references.md#ref-6)). The third stage — RL/preference alignment — is smaller still and rarely even reported in tokens: DeepSeek-Coder has no RL stage and Qwen2.5-Coder's DPO data size is unreported, while CodeRL's executable-reward RL runs over only $5{,}000$ APPS problems — the training half of APPS's 50-50 train/test split (Introductory 2639 + Interview 2000 + Competition 361) <!-- cite:19 --> [[19]](references.md#ref-19) — a count of problems, not a token budget. *Right — how serving compute is saved:* generating $T$ tokens without a key/value cache reprocesses the growing prefix every step (token $t$ costs $t$ block-passes, total $T(T{+}1)/2 = O(T^2)$), whereas the KV cache makes each new token cost one block pass (total $T = O(T)$). At $T = 8192$ that is $33.6$M versus $8192$ block-passes — a $\approx\!4097\times$ reduction (ratio $(T{+}1)/2$), the quantitative form of "one block pass rather than reprocessing the whole prefix." (The per-token attention-to-past cost is $O(t)$ either way; the cache removes the prefix *recomputation*.) Deterministic; values reproduced in the `.json` sidecar. Regenerate via `surveys/llms-for-coding/figures/training-and-serving-economics.py`.

<!-- sec:3.5.1 -->
#### <a id="sec-3.5.1"></a>3.5.1 Why the KV Cache Makes Generation Linear, Not Quadratic

<a id="p-351-why-the-kv-cache-makes-generation-linear-not-quadratic-1"></a><!-- para:351-why-the-kv-cache-makes-generation-linear-not-quadratic-1 --> The serving recursion of <!-- secref:3.5 -->[§3.5](#sec-3.5) caches the keys and values so that "each new token costs one block pass rather than reprocessing the whole prefix," and Figure 3.5 plots the resulting collapse. Here is why it holds — a counting argument, then the property that makes the saving lossless.

<a id="p-351-why-the-kv-cache-makes-generation-linear-not-quadratic-2"></a><!-- para:351-why-the-kv-cache-makes-generation-linear-not-quadratic-2 --> **The count.** Take a *block pass* — one token carried once through the transformer stack — as the unit. To emit the next token, a *cacheless* decoder calls the forward pass afresh on the entire running sequence; when that sequence has length $t$ it pushes all $t$ tokens through the stack, costing $t$ block passes. Summed over the generation, $\sum_{t=1}^{T} t = \tfrac{T(T+1)}{2} = \Theta(T^2)$. A *cached* decoder instead computes one new token per step, $\sum_{t=1}^{T} 1 = T = \Theta(T)$. The left panel of Figure 3.6 is exactly this: the cacheless cost is the area of a triangle, the cached cost its diagonal.

<a id="p-351-why-the-kv-cache-makes-generation-linear-not-quadratic-3"></a><!-- para:351-why-the-kv-cache-makes-generation-linear-not-quadratic-3 --> **Why the prefix may be reused.** The forward pass is a pure function of its input, so the cacheless decoder recomputes, every step, quantities it already produced — and those quantities do not change. Under the causal mask, position $j$ attends only to positions $\le j$, so its key $\mathbf{k}_j$ and value $\mathbf{v}_j$ — at every layer — depend only on $x_1,\dots,x_j$ and are *invariant* to any token appended later. They are therefore safe to store once and reuse. Because attention consumes the prefix solely through its keys and values, caching $\{\mathbf{k}_j,\mathbf{v}_j\}$ is all that is needed: each step computes the one new token's $\mathbf{q},\mathbf{k},\mathbf{v}$, attends against the cached pairs, and appends its own — one block pass. The right panel of Figure 3.6 confirms the invariance numerically: recomputing a prefix's keys inside a longer sequence reproduces the cached ones to the bit ($\max_j \lVert\Delta\mathbf{k}_j\rVert = 0$ exactly), so caching is lossless, not an approximation.

<a id="p-351-why-the-kv-cache-makes-generation-linear-not-quadratic-4"></a><!-- para:351-why-the-kv-cache-makes-generation-linear-not-quadratic-4 --> ![Two panels. Left: a lower-triangular grid with rows as sequence positions and columns as decode steps; the diagonal cells are green (one new block-pass per step) and the cells below the diagonal are grey (the prefix), illustrating that a cacheless decoder fills the whole triangle while the KV cache computes only the diagonal. Right: paired bars of the layer-two key norm at each position computed for a length-six prefix and for the same prefix inside a length-seven sequence; the bars coincide exactly on the shared positions and only position seven is new.](figures/kv-cache-mechanism.svg)

<!-- sec:3.5.1-figure-3 -->
<a id="p-351-why-the-kv-cache-makes-generation-linear-not-quadratic-5"></a><!-- para:351-why-the-kv-cache-makes-generation-linear-not-quadratic-5 --> <a id="sec-3.5.1-figure-3"></a>**Figure 3.6.** Why caching is both necessary and lossless. *Left:* the per-step work as a tableau (rows are sequence positions, columns are decode steps). A cacheless decoder recomputes the whole prefix each step, filling the lower-triangular region — $\sum_{s=1}^{T} s = T(T{+}1)/2$ block passes (here $36$ at $T = 8$); the KV cache computes only the green diagonal, one new block pass per step ($T = 8$), and reads the grey prefix from the cache. *Right:* causal invariance, computed through a two-layer causal self-attention stack. The layer-2 key at each prefix position is identical whether computed for the length-6 prefix alone or inside the length-7 sequence (the paired bars coincide; $\max_j \lVert\Delta\mathbf{k}_j\rVert = 0$ exactly), because the causal mask makes key $j$ depend only on tokens $\le j$; only position 7 is new. The cached keys are therefore exactly what a recompute would produce. Regenerate via `surveys/llms-for-coding/figures/kv-cache-mechanism.py`.

<a id="p-351-why-the-kv-cache-makes-generation-linear-not-quadratic-6"></a><!-- para:351-why-the-kv-cache-makes-generation-linear-not-quadratic-6 --> **What it buys.** The cache trades memory for compute: it stores $O(T)$ key/value vectors per layer so that per-token compute stays flat instead of growing with the prefix. One honest qualification — the new token still *scans* all $t$ cached keys, an $O(t)$ score computation per step that sums to $\Theta(T^2)$ with or without the cache; what the cache eliminates is the prefix *recomputation* (the projection and feed-forward work the block-pass count tracks), the term that would otherwise make decoding quadratic. The serving optimizations that attack the remaining scan and the cache's memory footprint are the subject of Section 10.

<a id="p-351-why-the-kv-cache-makes-generation-linear-not-quadratic-7"></a><!-- para:351-why-the-kv-cache-makes-generation-linear-not-quadratic-7 --> **Intuition (signal processing).** This is the difference between recomputing a statistic over the entire history at each step and maintaining it recursively — a from-scratch convolution over all past samples versus a recursive filter that carries its state forward.

<!-- sec:3.6 -->
### <a id="sec-3.6"></a>3.6 Scaling Laws

<a id="p-36-scaling-laws-1"></a><!-- para:36-scaling-laws-1 --> How good is a model before we train it? Remarkably, the answer is *predictable*. Across many orders of magnitude, the pretraining cross-entropy loss falls as a **power law** in each of three resources held non-bottlenecked: ==blue: the number of (non-embedding) parameters $N$==, ==blue: the dataset size $D$ in tokens==, and ==blue: the training compute $C$==. Kaplan et al. fit <!-- cite:55 --> [[55]](references.md#ref-55)
