...
- <a id="p-53-three-deployment-targets-2"></a><!-- para:53-three-deployment-targets-2 --> **Inline autocomplete** is the original Copilot setting: the model completes code at the cursor, conditioned on the surrounding file. It depends on **FIM** (the cursor has code on both sides) and is brutally **latency-bound** — a completion must appear in well under a second — which favors smaller models and the serving tricks of Section 10. DeepSeek-Coder and Qwen2.5-Coder both tie FIM directly to code-completion tooling; StarCoder ships the same capability but frames it more broadly, as enabling infilling tasks that go *beyond* left-to-right completion — return-type prediction and docstring generation among them <!-- cite:8 --> [[8]](references.md#ref-8), <!-- cite:10 --> [[10]](references.md#ref-10), <!-- cite:11 --> [[11]](references.md#ref-11).
