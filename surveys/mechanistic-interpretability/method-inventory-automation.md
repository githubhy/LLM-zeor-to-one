...
2. **Local replacement model.** For one fixed prompt, swap each true MLP for its CLT reconstruction, **freeze attention patterns** at their real values (attention is left nonlinear because it is harder to sparsify), and add a per-layer **error node** equal to (true output $-$ CLT output). By construction this replacement reproduces the original model's output *exactly on that input* — "local" meaning input-specific and faithful even where the CLT is imperfect.
