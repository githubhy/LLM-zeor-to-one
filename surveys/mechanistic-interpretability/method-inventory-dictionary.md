...
<a id="p-65-sae-pathologies-load-bearing-1"></a><!-- para:65-sae-pathologies-load-bearing-1 --> Six well-documented failure modes bound what SAE features can be trusted to mean. The first five are properties of the *dictionary*; the sixth, added in 2026, is a property of the **instrument used to certify that a feature was found at all**, and it therefore applies on top of the others rather than beside them:
...
- **Causally inert "recovered" features** <!-- cite:87 --> [[87]](references.md#ref-87) — a decoder atom can match a ground-truth feature direction almost perfectly by cosine similarity and still **never fire** when that feature is present. In a well-trained TopK SAE, **9%** of matched features (2 of 22) are inert in this sense, both at cosine above 0.9999, over 500 feature-present samples. The pathology is not in the dictionary but in the *metric*: cosine matching is the standard bar for "the SAE recovered feature $X$", and it can be maximally confident and causally wrong. Scope the number carefully — it is measured in a synthetic toy model with known ground truth, offered by its authors as "a calibration point for the easy case, not a benchmark for a production model"; the same paper's real-model census (GPT-2-small, 83 hand-authored concepts) finds 1 of 7 matched features inert and explicitly declines to treat the two regimes as comparable. Its headline 77% figure is a *degraded* SAE at a *matched-pair* denominator, not a recovery-bar denominator — see § <!-- secxref:12.2 -->[§12.2](state-of-the-art-and-practice.md#sec-12.2).
