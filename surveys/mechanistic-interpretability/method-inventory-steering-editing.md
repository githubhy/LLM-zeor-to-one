...
<a id="p-74-rome-locating-and-editing-a-fact-load-bearing-5"></a><!-- para:74-rome-locating-and-editing-a-fact-load-bearing-5 --> **Limits & epistemic tag.** Single-fact; fragile under iterated edits (§ <!-- secref:7.5 -->[§7.5](#sec-7.5)); and — the deep caveat — **editing success does not validate the localization (<!-- secxref:Q.6 -->[§Q.6](appendix-q-reader-questions.md#sec-Q.6))**: Hase et al. <!-- cite:54 --> [[54]](references.md#ref-54) find edit success is ~equally good at layers far from the causally-traced site (a regression of edit success on layer gets $R^2\approx 0.585$; adding the causal-tracing effect size improves it by at most 0.03), so "ROME works at layer $\ell$" is *not* evidence the fact is "stored at" $\ell$. *A landmark method whose main lesson is a methodological warning.*

<!-- sec:7.5 -->
### <a id="sec-7.5"></a>7.5 MEMIT — mass editing [load-bearing]

<a id="p-75-memit-mass-editing-load-bearing-1"></a><!-- para:75-memit-mass-editing-load-bearing-1 --> **One-line idea.** Spread the required residual change across a *range* of mid-layer MLPs and solve a batched least-squares insert (<!-- secxref:E.3 -->[§E.3](appendix-e-steering-and-editing-math.md#sec-E.3)), so thousands of facts can be edited at once without breaking the model.
...
<!-- sec:7.7 -->
### <a id="sec-7.7"></a>7.7 Knowing is not steering [load-bearing]

<a id="p-77-knowing-is-not-steering-load-bearing-1"></a><!-- para:77-knowing-is-not-steering-load-bearing-1 --> **One-line idea.** A direction that reads a property out of the residual stream perfectly can be nearly useless for pushing that property in — detection and control are different problems with different optimal directions, and the gap has now been measured.

<a id="p-77-knowing-is-not-steering-load-bearing-2"></a><!-- para:77-knowing-is-not-steering-load-bearing-2 --> **Placement & lineage.** This section exists because everything above it assumes the two coincide. Sections <!-- secref:7.1 -->[§7.1](#sec-7.1)–<!-- secref:7.3 -->[§7.3](#sec-7.3) build a direction from labelled contrasts and then *add* it; § <!-- secxref:4.1 -->[§4.1](method-inventory-observational.md#sec-4.1) builds a direction from labelled contrasts and then *reads* it. Appendix <!-- secxref:E.4 -->[§E.4](appendix-e-steering-and-editing-math.md#sec-E.4) shows from first principles that these need not be the same vector: the optimal *detector* is $\Sigma^{-1}(\boldsymbol{\mu}-\boldsymbol{\nu})$ and the natural *steering* displacement is $\boldsymbol{\mu}-\boldsymbol{\nu}$, and they coincide only when the class-mean difference is an eigenvector of the activation covariance. So a gap is predicted before any measurement.

<a id="p-77-knowing-is-not-steering-load-bearing-3"></a><!-- para:77-knowing-is-not-steering-load-bearing-3 --> **Stated result.** On Gemma-2-2B-it, hallucination-related uncertainty is linearly separable **perfectly** — AUC $= 1.000$ from layer 5, by linear probe, by a single attention head, and by difference-in-means — while the direction carrying that signal sits at $\cos = 0.12$, about $83^\circ$, from the direction that actually controls refusal <!-- cite:97 --> [[97]](references.md#ref-97). Interventions along the detection direction do not produce the behaviour: SAE feature amplification up to $50\times$ leaves refusal unchanged, and ablating the top 500 discriminative neurons produces no behavioural change. The geometry is not an artifact of alignment training — base and instruction-tuned models give $\cos = 0.1197$ and $0.1200$ — and it holds across four models at $\cos \in [0.12, 0.20]$. Rotating $15^\circ$ from detection toward the intervention direction partially bridges it, reaching 73% and 60% refusal on two held-out entity categories at 1.8% false positives ($N = 115$).

<a id="p-77-knowing-is-not-steering-load-bearing-4"></a><!-- para:77-knowing-is-not-steering-load-bearing-4 --> **Two caveats that constrain the claim, and this survey states both.** First, **the one scale-up datapoint on the fix is negative**: Gemma-2-9B-it "reproduces the geometry ($\cos = 0.13$) but resists steering — the same intervention is suppressed rather than amplified at 9B", and the authors report its geometry without a comparable intervention table. Whether larger models systematically damp this steering is open, so the *repair* is demonstrated only at 2B. Second, **the base-model result does not mean what the cosine alone suggests**: only the *axis* predates instruction tuning, not the signal. In the base model "the ordering is backwards — real entities project further along it, as if more uncertain than the fakes (gap $-23.2$)", and instruction tuning flips the sign (gap $+49.8$). Pretraining lays down the geometry; alignment supplies the polarity that makes it a usable detector.

<a id="p-77-knowing-is-not-steering-load-bearing-5"></a><!-- para:77-knowing-is-not-steering-load-bearing-5 --> **What it is not.** The angle is **not** an a-priori steerability oracle, and the paper is explicit in rejecting that reading of its own result: "the cosine is a signature of the gap, not an oracle for it." Steerability is a functional question — does the controlling direction also detect? — not a static angle to be measured once and consulted. The survey therefore records "detection $\neq$ control" as a demonstrated dissociation with a mechanism, not as a diagnostic anyone can currently apply in advance.

- <a id="p-77-knowing-is-not-steering-load-bearing-6"></a><!-- para:77-knowing-is-not-steering-load-bearing-6 --> Full derivation: Appendix <!-- secxref:E.4 -->[§E.4](appendix-e-steering-and-editing-math.md#sec-E.4) (why the detector and the steering displacement differ, and the exact condition under which they coincide).
- Worked example: the $\cos = 0.12$ / AUC $= 1.000$ dissociation above.

<a id="p-77-knowing-is-not-steering-load-bearing-7"></a><!-- para:77-knowing-is-not-steering-load-bearing-7 --> **Limits & epistemic tag.** Sample sizes are modest ($N = 20$–150 per condition; $N = 15$ per model for the cross-model causal replication, which the authors call exploratory), only decoder-only models from 1B to 9B were tested, and the deep dive is a single behaviour plus one positive control. *A sharp, mechanistically explained dissociation on one behaviour at small scale — strong enough to retire the assumption that a good probe direction is a good steering direction, not yet strong enough to predict which directions will steer.*
