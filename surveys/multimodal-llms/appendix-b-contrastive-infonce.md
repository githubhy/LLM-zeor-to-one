...
> <a id="p-b3-the-mutual-information-lower-bound-and-why-batch-size-matters-4"></a><!-- para:b3-the-mutual-information-lower-bound-and-why-batch-size-matters-4 --> **Note — the middle inequality of Equation <!-- ref:B-4 -->[(4)](#eq-4) is false pointwise and true in expectation, which is the only place it is used.** Read pointwise, $\log[1 + r(N-1)] \ge \log[rN]$ is equivalent (as $\log$ is monotone and $r>0$) to $1 + r(N-1) \ge rN$, i.e. to $r \le 1$ — the caption being more likely given the image than unconditionally. That holds *on average*, since that is what positive mutual information means, but it fails for individual pairs with positive probability, and the source writes the step as a bare inequality with no condition. It is worth knowing that the step is not the pointwise fact it resembles. But the patch that observation invites — bounding the pointwise loss by $\log\frac{N}{N-1}$ and weakening the conclusion to $I \ge \log(N-1) - \mathcal{L}_{\mathrm{NCE}}$ — is **unnecessary**, because the step never has to hold pointwise. Substituting $u = -\log r$, the quantity in question is $\phi(u) = \log(e^{u} + N - 1) - \log N$, a log-sum-exp and therefore **convex** in $u$, increasing, with $\phi(0) = 0$. Since $\mathbb{E}[u] = I(x_v;x_t) \ge 0$, Jensen for a convex function runs in the helpful direction: $\mathbb{E}\,\phi(u) \ge \phi(\mathbb{E}u) = \phi(I) \ge \phi(0) = 0$. So the step is valid **for every distribution arising in this setting, with no side condition**, and Equation <!-- ref:B-5 -->[(5)](#eq-5) stands exactly as published — with equality only in the degenerate case $I = 0$. The same argument gives something sharper than either form, $\log N - \mathcal{L}_{\mathrm{NCE}} \le I + \log\frac{N}{e^{I} + N - 1}$, whose right-hand side is at most $I$ (the bound) and tends to $\log N$ as $I$ grows (the ceiling discussed below) — one line delivering both. *The general lesson is worth more than the result: an inequality used only under an expectation should be checked under that expectation, not pointwise, or a perfectly sound derivation gets "repaired" into a weaker one.*
