...
<a id="p-f2-the-whisper-encoder-and-a-30-second-chunk-end-to-end-4"></a><!-- para:f2-the-whisper-encoder-and-a-30-second-chunk-end-to-end-4 --> Two observations make this number worth carrying. First, $1500$ positions is *comparable to an image* — but only on a matched basis, and the basis is easy to get wrong. Compare like with like: $1500$ is the count at the **encoder output**, before any connector, and the matching image quantity is a $336$-px frame's $(336/14)^2 = 576$ patch tokens at the vision encoder's output. On that basis half a minute of speech costs $1500/576 = 2.6$ images. On the **LLM-visible** basis the answer is completely different, because audio passes through a resampler and an image often does not: SALMONN's window-level Q-Former (<!-- secref:F.4 -->[§F.4](#sec-F.4)) delivers $88$ tokens for those same $30$ seconds, so the LLM sees $88/576 = 0.15$ images' worth — a factor of $17$ apart from the encoder-side reading, and in the opposite direction. Both numbers are correct and they answer different questions; quoting one while thinking of the other is the single easiest mistake to make in this appendix. Audio is cheap per second and expensive per minute — the opposite of the intuition that sound is the "lighter" modality — and that statement holds on either basis, which is why it is safe to carry. Second, the $30$ s figure is a hard architectural boundary rather than a default: Whisper models "are trained on 30-second audio chunks and cannot consume longer audio inputs at once" <!-- cite:19 -->[[19]](#ref-19), because the encoder's positional embedding table is exactly this long. Long-form transcription is therefore a *chunking* problem layered on top of the model, which is where its long-form failure modes come from.
...
<a id="p-f4-two-encoders-one-sequence-concatenation-and-window-level-resampling-4"></a><!-- para:f4-two-encoders-one-sequence-concatenation-and-window-level-resampling-4 --> This makes the output length *linear in duration* rather than constant, which is the right call: audio content grows with time, and a constant budget would degrade without bound. With $N = 1$ query and $L = 17$ frames, about $0.33$ s per window <!-- cite:21 -->[[21]](#ref-21), the compression from Equation <!-- ref:F-3 -->[(3)](#eq-3) is roughly $1500 \to 88$, a factor of $17$, and the token rate falls to about $2.9$ tokens per second of audio. That is *comparable to*, not clearly cheaper than, the cost of the same speech transcribed: English at $r$ words per minute costs roughly $(r/60)\tau$ tokens per second with $\tau \approx 1.3$–$1.4$ subword tokens per word, so break-even against $2.9$ sits at about $126$–$135$ wpm — inside the range normally called conversational. Whether streaming the audio is cheaper than transcribing it therefore depends on which end of "normal" the speaker is at, which makes it a design consideration rather than a result.
