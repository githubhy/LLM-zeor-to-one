...
|---|---|---|
| Documents, OCR, charts, small text | high-resolution early fusion (LLaVA-NeXT AnyRes or Qwen2-VL native), MLP projector, *no* aggressive token reduction | detail lives below the patch scale; it needs tokens (<!-- secxref:2.2 -->[§2.2](fundamentals.md#sec-2.2), <!-- secxref:8.1 -->[§8.1](inference-and-serving.md#sec-8.1)) |
| Many images / few-shot / interleaved prompts | deep fusion (Flamingo-style) or a token-reducing resampler | per-image token cost must stay bounded (<!-- secxref:3.3 -->[§3.3](architecture-building-blocks.md#sec-3.3), <!-- secxref:3.4 -->[§3.4](architecture-building-blocks.md#sec-3.4)) |
| The LLM must stay frozen (no fine-tuning budget) | deep fusion (Flamingo) or the Q-Former bridge (BLIP-2) | both keep the LM's weights untouched (<!-- secxref:3.5 -->[§3.5](architecture-building-blocks.md#sec-3.5)) |
| Any-to-any generation, unification first | native-token AR (Chameleon / Emu3) | one vocabulary, one loss reads and writes (<!-- secxref:6.1 -->[§6.1](multimodal-generation.md#sec-6.1)) |
| Any-to-any generation, image quality first | a diffusion hybrid (Transfusion) | no lossy tokenizer in the image path (<!-- secxref:6.2 -->[§6.2](multimodal-generation.md#sec-6.2)) |
| Strong at *both* understanding and generation | decoupled encoders (Janus) | one encoder is a compromise between the two (<!-- secxref:6.3 -->[§6.3](multimodal-generation.md#sec-6.3)) |
| Edge / tight token or memory budget | resampler connector plus LLM-side pruning (FastV) | fewest visual tokens carried, then fewest attended (<!-- secxref:8.2 -->[§8.2](inference-and-serving.md#sec-8.2)) |
| Audio understanding | a Whisper encoder plus a projector (Qwen-Audio) or window-Q-Former (SALMONN) | reuse the pretrained speech front-end (<!-- secxref:7.1 -->[§7.1](modality-breadth.md#sec-7.1)) |
